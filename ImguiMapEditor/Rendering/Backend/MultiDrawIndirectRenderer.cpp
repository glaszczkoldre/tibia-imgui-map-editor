...
bool MultiDrawIndirectRenderer::initialize() {
    ...
    // No preprocessor check needed - GLAD sets the function pointer at runtime
    available_ = (glMultiDrawElementsIndirect != nullptr);
    
    if (!available_) {
        spdlog::warn("MultiDrawIndirectRenderer: glMultiDrawElementsIndirect not available (requires GL 4.3+)");
        return false;
    }
    
    spdlog::info("MultiDrawIndirectRenderer: GL 4.3+ detected, MDI enabled");
    ...
    initialized_ = true;
    spdlog::info("MultiDrawIndirectRenderer: Initialized (max {} commands)", MAX_COMMANDS);
...
void MultiDrawIndirectRenderer::addDrawCommand(GLuint count, GLuint instanceCount,
                                                GLuint firstIndex, GLuint baseVertex,
                                                GLuint baseInstance) {
    if (commands_.size() >= MAX_COMMANDS) {
        spdlog::warn("MultiDrawIndirectRenderer: Max commands reached, ignoring");
        return;
    }

    if (instanceCount == 0) return;  // Skip empty draws
...
void MultiDrawIndirectRenderer::upload() {
    if (commands_.empty() || !initialized_) return;

    glBindBuffer(GL_DRAW_INDIRECT_BUFFER, command_buffer_);
    glBufferSubData(GL_DRAW_INDIRECT_BUFFER, 0,
                    commands_.size() * sizeof(DrawElementsIndirectCommand),
                    commands_.data());
    // Keep buffer bound for execute()
}

void MultiDrawIndirectRenderer::execute() {
    if (commands_.empty() || !available_ || !initialized_) return;
