...
/**
 * Transform V710 (7.10-7.30) raw flag to canonical flag.
 ...
 */
uint8_t DatReaderV710::transformFlag(uint8_t raw) {
    uint8_t flag = raw;
    
    // Shift flags 1-15 up by 1 (no GROUND_BORDER in 7.10)
    if (flag > 0 && flag <= 15) {
        flag += 1;
    } else if (flag == 16) {
        flag = HAS_LIGHT;
...
bool DatReaderV710::handleSpecificFlag(uint8_t flag, ClientItem& item, BinaryReader& reader) {
    if (flag == HAS_OFFSET) {
        item.has_offset = true;
        // V7.10-7.30: Fixed offset, NOT read from file
        item.offset_x = 8;
        item.offset_y = 8;
        return true;
    }
