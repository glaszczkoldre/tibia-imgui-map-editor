...
for /f "tokens=2 delims=:" %%a in ('findstr /C:"builtin-baseline" "%PROJECT_ROOT%\ImguiMapEditor\vcpkg.json"') do (
    set "RAW=%%a"
)
...
cmake -S "!PROJECT_ROOT!\ImguiMapEditor" -B "!IDE_DIR!" -G "!VS_GENERATOR!" -A x64 "-DCMAKE_TOOLCHAIN_FILE=!VCPKG_DIR!\scripts\buildsystems\vcpkg.cmake" "-DVCPKG_TARGET_TRIPLET=x64-windows" "-DCMAKE_CONFIGURATION_TYPES=Release;RelWithDebInfo;Debug"
...
echo %GREEN%========================================================%RESET%
echo.
echo   Solution file:  %BOLD%%IDE_DIR%\TibiaMapEditor.sln%RESET%
echo   vcpkg root:     %VCPKG_DIR%
echo.
echo   To open the solution, run:
echo     %CYAN%start "" "%IDE_DIR%\TibiaMapEditor.sln"%RESET%
echo.
pause
